...
def split_classify_dataset(source_dir: str | Path, train_ratio: float = 0.8) -> Path:
    """
    Split classification datasets into train and valid directories in a new directory.

    Creates a new directory '{source_dir}_split' with train/valid subdirectories, preserving the original class
    ...
    Args:
        source_dir (str | Path): Path to classification datasets root directory.
    ...
    Examples:
        Split datasets with default 80/20 ratio
    ...
    train_path, val_path = split_path / "train", split_path / "valid"
    ...
    LOGGER.info(f"Splitting {source_path} ({stats}) into {train_ratio:.0%} train, {1 - train_ratio:.0%} valid...")
...
def autosplit(
    ...
    """
    Automatically split a datasets into train/valid/test splits and save the resulting splits into autosplit_*.txt files.
