...
def check_file_speeds(
    ...
    This function tests the access speed of datasets files by measuring ping (stat call) time and read speed.
...
def check_det_dataset(dataset: str, autodownload: bool = True) -> dict[str, Any]:
    ...
    for k in "train", "valid":
        if k not in data:
            ...
            LOGGER.warning("renaming data YAML 'validation' key to 'valid' to match YOLO format.")
...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    ...
    Args:
        ...
        split (str, optional): The split of the datasets. Either 'valid', 'test', or ''.

    Returns:
        (dict[str, Any]): A dictionary containing the following keys:

            - 'train' (Path): The directory path containing the training set of the datasets.
            - 'valid' (Path): The directory path containing the validation set of the datasets.
    ...
    if split == "valid" and not val_set:
        LOGGER.warning("Dataset 'split=valid' not found, using 'split=test' instead.")
        val_set = test_set
    elif split == "test" and not test_set:
        LOGGER.warning("Dataset 'split=test' not found, using 'split=valid' instead.")
        test_set = val_set
    ...
    return {"train": train_set, "valid": val_set, "test": test_set, "nc": nc, "names": names, "channels": 3}
