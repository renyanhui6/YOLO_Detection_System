...
class WorldTrainerFromScratch(WorldTrainer):
    ...
    Attributes:
        ...
        data (dict): Final processed data configuration containing train/valid paths and metadata.
    ...
    def build_dataset(self, img_path, mode="train", batch=None):
        ...
        Args:
            ...
            mode (str): 'train' mode or 'valid' mode, allowing customized augmentations for each mode.
    ...
    def get_dataset(self):
        ...
        assert data_yaml.get("valid", False), "validation datasets not found"  # lvis.yaml
        data = {k: [check_det_dataset(d) for d in v.get("yolo_data", [])] for k, v in data_yaml.items()}
        assert len(data["valid"]) == 1, f"Only support validating on 1 datasets for now, but got {len(data['valid'])}."
        ...
        # assign the first valid datasets as currently only one validation set is supported
        data["valid"] = data["valid"][0]
        final_data["valid"] = final_data["valid"][0]
        # NOTE: to make training work properly, set `nc` and `names`
        final_data["nc"] = data["valid"]["nc"]
        final_data["names"] = data["valid"]["names"]
        # NOTE: add path with lvis path
        final_data["path"] = data["valid"]["path"]
        final_data["channels"] = data["valid"]["channels"]
